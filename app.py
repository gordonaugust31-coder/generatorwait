...
def init_state():
    defaults = {
        "step": 0, "messages": [], "format": None, "num_sites": 1,
        "geo": "", "language": "", "has_domains": None, "domains": [],
        "contacts_raw": "", "contacts": {}, "theme_mode": None,
        "theme_niche": "", "themes": [], "has_seo": None, "seo_keywords": "",
        "has_stopwords": None, "stop_words": "", "has_extra": None,
        "extra_requirements": "", "generating": False, "gen_result": None,
        "api_key": "",
    }
...
def bot(text): add_msg("assistant", text)
def usr(text): add_msg("user", text)
def reset_all():
...
def validate_domain(d):
    ...
    if re.match(r'^[a-z0-9]([a-z0-9\-]*[a-z0-9])?(\.[a-z]{2,})+$', d): return d
    return None
...
def parse_contacts(text, domains):
    ...
    for line in text.strip().split("\n"):
        ...
        if m:
            ...
            if dom: contacts[dom] = _extract_contact(m.group(2))
    if not contacts:
        parts = _extract_contact(" ".join(text.strip().split("\n")))
        for dom in domains: contacts[dom] = parts.copy()
    if contacts:
        default = list(contacts.values())[0]
        for dom in domains:
            if dom not in contacts: contacts[dom] = default.copy()
    return contacts

def _extract_contact(text):
    phone, email, address = "", "", text
    em = re.search(r'[\w\.\-+]+@[\w\.\-]+\.\w+', text)
    if em: email = em.group(0); address = address.replace(email, "")
    ph = re.search(r'[\+]?[\d\s\-\(\)]{7,20}', text)
    if ph: phone = ph.group(0).strip(); address = address.replace(phone, "")
...
def call_claude(api_key, system, prompt, max_tokens=8000):
    ...
    for attempt in range(3):
        try:
            ...
            err = str(e).lower()
            if ("rate" in err or "overloaded" in err or "529" in err) and attempt < 2:
...
def generate_themes(api_key, niche, count, language):
    r = call_claude(api_key,
        ...
        f"Language: {language}. Return JSON array of {count} strings.", max_tokens=2000)
...
def generate_nav(api_key, niche, language, stop_words):
    r = call_claude(api_key,
        ...
        f"Slugs: lowercase ASCII hyphens only.", max_tokens=500)
    return json.loads(_clean_json(r))
...
def generate_page(api_key, template_html, page_type, cfg, nav, stop_words, seo_kw, extra):
    ...
    system = (
        ...
        f"html lang=\"{lc}\"\nOutput ONLY complete HTML, no markdown/fences."
    )
...
def split_page(html):
    ...
    if m1 and m2:
        return html[:m1.end()], html[m1.end():m2.start()], html[m2.start():]
    return html, "", ""
...
def do_replace(html, cfg, nav, fmt):
    ...
    for old, new in reps: html = html.replace(old, new)
    return html
...
def build_one_site(api_key, site_cfg, tpl, fmt, stop_words, seo_kw, extra, log_fn=None):
    ...
    # Step 1: Generate index.php FULLY (translated header/footer)
    if log_fn: log_fn("📝 Home (повна генерація)...")
    ...
    try:
        ...
        if log_fn: log_fn("✅ Home")
    except Exception as e:
        if log_fn: log_fn(f"❌ Home: {str(e)[:200]}")
    time.sleep(2)

    # Step 2: Extract header/footer from index for reuse
    shared_header, shared_footer = None, None
    if index_html:
        before, _, after = split_page(index_html)
        shared_header = before
        shared_footer = after

    # Step 3: Remaining pages — only <main> (2x faster)
    remaining = [
    ...
    for tpl_name, page_type in remaining:
        ...
        try:
            ...
            if log_fn: log_fn(f"❌ {page_type}: {str(e)[:200]}")
        time.sleep(2)
    ...
    # Static assets (CSS, JS, images, favicons)
...
def run_generation(api_key):
    ...
    def log(msg):
        ...
        with log_area:
            for entry in logs[-6:]: st.markdown(entry)
...
with st.sidebar:
    ...
    st.divider()
    st.caption("Модель: claude-sonnet-4-6")
...
if not TEMPLATE_ZIP_PATH.exists():
    st.error("❌ `template.zip` не знайдено!"); st.stop()
...
if st.session_state.generating:
    ...
    if not ak:
        st.error("❌ API ключ не знайдено!"); st.session_state.generating = False
    else:
        try:
            ...
            st.error(f"❌ Помилка: {e}"); st.session_state.generating = False
    st.stop()
...
if st.session_state.gen_result:
    ...
    st.download_button("⬇️ Завантажити ZIP", data=st.session_state.gen_result,
        file_name=f"sites_{datetime.now().strftime('%Y%m%d_%H%M%S')}.zip",
        mime="application/zip", type="primary", use_container_width=True)
    if st.button("🆕 Нова генерація"): reset_all(); st.rerun()
    st.stop()
...
if step == 1:
    ...
    with c1:
        if st.button("✅ Так, є домени", use_container_width=True, key="s5_yes"):
            usr("Так, є домени")
            bot(f"📋 Надішліть список доменів.\n\nМожна по одному в рядку або через кому.\n\nПотрібно: **{n}**. Коли закінчите — натисніть «✅ Готово».")
    ...
    with c1:
        if st.button("✅ Готово", key="s51_done", use_container_width=True):
            ...
            if not doms:
                st.error("❌ Не бачу доменів. Приклад: `example.com, site.net`")
            else:
                if len(doms) < need:
                    for i in range(len(doms), need): doms.append(f"site-{i+1:03d}.example.com")
                elif len(doms) > need: doms = doms[:need]
                st.session_state.domains = doms
                usr(f"Домени: {', '.join(doms[:5])}{'...' if len(doms)>5 else ''}")
                bot(f"✅ Доменів: **{len(doms)}**. ✅ Достатньо.")
                st.session_state.step = 6; st.rerun()
    with c2:
        if st.button("⏭️ Пропустити", key="s51_skip", use_container_width=True):
            doms = [f"site-{i+1:03d}.example.com" for i in range(need)]
            st.session_state.domains = doms; usr("Пропустити")
            bot(f"✅ Створено {need} тимчасових доменів.")
            st.session_state.step = 6; st.rerun()

elif step == 6:
    bot("📞 **Контакти**: надішліть телефон / адресу / пошту.\n\nФормат: `domain.com: адреса, +39..., email@...`\nабо просто список контактів.\n\nКоли закінчите — натисніть «✅ Готово».")
    st.session_state.step = 61; st.rerun()

elif step == 61:
    st.markdown("##### 📞 Контакти")
    ct = st.text_area("Контактні дані", placeholder="domain.com: Via Roma 10, +39 02 123, info@domain.com", height=150, key="s61_area")
    uf = st.file_uploader("Або .txt файл", type=["txt"], key="s61_file")
    if st.button("✅ Готово", key="s61_done", use_container_width=True):
        text = ct
        if uf: text = uf.read().decode("utf-8", errors="replace")
        if not text.strip(): st.error("❌ Введіть контактні дані.")
        else:
            contacts = parse_contacts(text, st.session_state.domains)
            st.session_state.contacts = contacts
            usr(f"Контакти ({len(contacts)} записів)")
            bot("✅ Контакти збережені."); st.session_state.step = 7; st.rerun()

elif step == 7:
    st.markdown("##### 📌 Тематика сайтів")
    c1, c2 = st.columns(2)
    with c1:
        if st.button("🧠 Згенерувати різні теми", use_container_width=True, key="s7_gen"):
            usr("Згенерувати теми")
            bot("Опишіть область / нішу для генерації тем:")
            st.session_state.theme_mode = "generate"; st.session_state.step = 71; st.rerun()
    with c2:
        if st.button("📝 Вказати вручну", use_container_width=True, key="s7_manual"):
            usr("Вказати вручну")
            bot("Вкажіть теми через `;`.\nПриклад: `Forex education; Personal finance; Budget planning`")
            st.session_state.theme_mode = "manual"; st.session_state.step = 72; st.rerun()

elif step == 71:
    st.markdown("##### 🧠 Опишіть нішу")
    niche = st.text_input("Ніша", placeholder="здорове харчування, фітнес...", key="s71_niche")
    if st.button("🧠 Згенерувати", key="s71_go", use_container_width=True) and niche.strip():
        ak = st.session_state.get("api_key","")
        if not ak: st.error("❌ Вкажіть API ключ!")
        else:
            usr(niche.strip())
            with st.spinner("Генерую теми..."):
                try:
                    ...
                    bot(f"✅ Згенеровано {len(themes)} тем:\n\n{preview}")
                    st.session_state.step = 8; st.rerun()
                except Exception as e: st.error(f"❌ Помилка: {e}")

elif step == 72:
    st.markdown("##### 📝 Теми")
    tt = st.text_area("Теми (через `;`)", placeholder="Yoga studio; CrossFit gym", key="s72_area")
    if st.button("✅ Готово", key="s72_done", use_container_width=True) and tt.strip():
        raw = [t.strip() for t in tt.split(";") if t.strip()]
        if not raw: st.error("❌ Вкажіть хоча б одну тему.")
        else:
            n = st.session_state.num_sites
            themes = (raw * (n // len(raw) + 1))[:n]
            st.session_state.themes = themes
            usr(f"Теми: {'; '.join(raw)}")
            bot(f"✅ {len(raw)} тем → {n} сайтів.")
            st.session_state.step = 8; st.rerun()

elif step == 8:
    st.markdown("##### 🔑 SEO ключові слова?")
    c1, c2 = st.columns(2)
    with c1:
    ...
    with c2:
        if st.button("❌ Ні", use_container_width=True, key="s8_no"):
            ...
            bot("✅ Без SEO ключових слів."); st.session_state.step = 9; st.rerun()

elif step == 81:
    kw = st.text_input("Ключові слова", placeholder="keyword1, keyword2", key="s81_kw")
    if st.button("✅ Готово", key="s81_done", use_container_width=True) and kw.strip():
        ...
        bot("✅ Збережено."); st.session_state.step = 9; st.rerun()
    ...
    with c2:
        if st.button("❌ Ні", use_container_width=True, key="s9_no"):
            ...
            bot("✅ Без стоп-слів."); st.session_state.step = 10; st.rerun()

elif step == 91:
    sw = st.text_input("Стоп-слова", placeholder="cheap, free, discount", key="s91_sw")
    if st.button("✅ Готово", key="s91_done", use_container_width=True) and sw.strip():
        usr(sw.strip()); st.session_state.stop_words = sw.strip()
        bot("✅ Збережено."); st.session_state.step = 10; st.rerun()

elif step == 10:
    st.markdown("##### 📋 Додаткові вимоги?")
    c1, c2 = st.columns(2)
    with c1:
        if st.button("✅ Так", use_container_width=True, key="s10_yes"):
            usr("Так"); st.session_state.has_extra = True
            bot("Опишіть вимоги:"); st.session_state.step = 101; st.rerun()
    with c2:
        if st.button("❌ Ні", use_container_width=True, key="s10_no"):
            usr("Ні"); st.session_state.has_extra = False
            bot("✅ Все зібрано! Перевірте підсумок і запускайте 🚀")
            st.session_state.step = 11; st.rerun()

elif step == 101:
    ex = st.text_area("Вимоги", placeholder="Формальний тон, FAQ секція...", key="s101_ex")
    if st.button("✅ Готово", key="s101_done", use_container_width=True) and ex.strip():
        ...
        bot("✅ Збережено. Перевірте підсумок і запускайте 🚀")
        st.session_state.step = 11; st.rerun()
    ...
    st.markdown(f"**Сайтів:** {s.num_sites}")
    st.markdown(f"**Гео:** {s.geo} | **Мова:** {s.language}")
    dp = ', '.join(s.domains[:5])
    if len(s.domains)>5: dp += f"... (+{len(s.domains)-5})"
    st.markdown(f"**Домени:** {dp}")
    st.markdown(f"**Контакти:** {len(s.contacts)} | **Теми:** {len(set(s.themes))} унікальних")
    if s.has_seo: st.markdown(f"**SEO:** {s.seo_keywords[:80]}")
    ...
    st.markdown("---")
    est = s.num_sites * 12
    st.info(f"⏱️ ~{est} хв ({s.num_sites} сайтів × 9 сторінок)")

    ak = st.session_state.get("api_key","")
    if not ak: st.error("⚠️ Вкажіть API ключ!")
    else:
        if st.button("🚀 ГЕНЕРУВАТИ САЙТИ", type="primary", use_container_width=True, key="s11_gen"):
            st.session_state.generating = True; st.rerun()
